...
def collect_data(params={}):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Fetched {len(stats)} player season stats from the API.")
            return stats
        else:
...
def main():   
    ...
    while i<31:
        if (i == 3): 
            query_params = {"team": 31 ,"season":"2024"}
            playerSeason_data = collect_data(query_params)
            insert_data_into_mongodb(playerSeason_data)
            i = i + 1
        elif (i == 12):
            query_params = {"team": 38 ,"season":"2024"}
            playerSeason_data = collect_data(query_params)
            insert_data_into_mongodb(playerSeason_data)
            i = i + 1
        elif (i==13):
            query_params = {"team": 40 ,"season":"2024"}
            playerSeason_data = collect_data(query_params)
            insert_data_into_mongodb(playerSeason_data)
            i = i + 1
        elif (i==18):
            query_params = {"team": 41 ,"season":"2024"}
            playerSeason_data = collect_data(query_params)
            insert_data_into_mongodb(playerSeason_data)
            i = i + 1
        else: 
            query_params = {"team": i ,"season":"2024"}
            playerSeason_data = collect_data(query_params)
            insert_data_into_mongodb(playerSeason_data)
            i = i + 1
